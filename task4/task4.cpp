...
{
public:
  float X = 0.0f, Y = 0.0f, theta = 0.0f, v_x = 0.0f, v_y = 0.0f, r = 0.0f, omega_f = 0.0f,
        omega_r = 0.0f, throttle = 0.0f, steering_angle = 0.0f, brakes = 0.0f;
};
...
public:
  float X_dot = 0.0f, Y_dot = 0.0f, phi_dot = 0.0f, vx_dot = 0.0f, vy_dot = 0.0f, r_dot = 0.0f,
        omega_dot_f = 0.0f, omega_dot_r = 0.0f;

  States_dot operator*=(float scalar)
  {
    return States_dot{this->X_dot * scalar,       this->Y_dot * scalar,      this->phi_dot * scalar,
                      this->vx_dot * scalar,      this->vy_dot * scalar,     this->r_dot * scalar,
                      this->omega_dot_f * scalar, this->omega_dot_r * scalar};
  }
...
public:
  ...
  States_dot calculateStatesDot(float thr, float ste, float bra, Kesi kesi)
  {
    ...
    Fxf = 2 * Tire.solveFx(kappa_f);
    Fxr = 2 * Tire.solveFx(kappa_r);
    ...
    states_dot_dyn.vx_dot =
      1 / m *
      (m * kesi.v_y * kesi.r + Fxf * cos(kesi_new.steering_angle) + Fxr - Tire.sgn(Fxr) * Frrr -
       Tire.sgn(Fxf) * Frrf * cos(kesi_new.steering_angle) - Fdrag -
       Fyf * sin(kesi_new.steering_angle));
    states_dot_dyn.vy_dot = 1 / m *
                            (-m * kesi.v_x * kesi.r + Fxf * sin(kesi_new.steering_angle) +
                             Fyf * cos(kesi_new.steering_angle) + Fyr -
                             Tire.sgn(Fxf) * Frrf * sin(kesi_new.steering_angle));

    states_dot_dyn.r_dot =
      1 / Iz *
      ((Fyf * cos(kesi_new.steering_angle) + Fxf * sin(kesi_new.steering_angle) -
        Tire.sgn(Fxf) * Frrf * sin(kesi_new.steering_angle)) *
         lf -
       Fyr * lr);
  ...
  void updatestate(float dt)
  {
    ...
    for (int i = 0; i < steps; ++i) {
      ...
      for (int j = 0; j < 50; ++j) {
        ...
        // RungeKutta integral
        //  outputFile<<"before vlf "<<vlf<<" "<<kesi_old.omega_f*Tire.r_eff()<<"
        //  "<<Tire.kappa(vlf,kesi_old.omega_f)<<"\n";

        kesi_new = rungeKutta(thr, ste, bra, dt);

        // outputFile<<" Long FDRV="<<Fdrv<<" FXR="<<Fxr<<" FXF="<<Fxf<<" FRRF="<<Frrf<<"
        // FBF="<<Fbf<<" FBR="<<Fbr<<"\n"; outputFile<<i*0.05+j*dt<<" FXR="<<Fxr<<" FXF="<<Fxf<<"
        // Fxr+Fxf="<<Fxr+Fxf<<" "<<"FDRV="<<Fdrv<<" "<<kesi_new.v_x<<"\n"; outputFile<<" vlf
        // "<<vlf<<" "<<kesi_old.omega_f*Tire.r_eff()<<" "<<Tire.kappa(vlf,kesi_old.omega_f)<<"\n";
      ...
      outputFile << "X:" << kesi_new.X << " Y:" << kesi_new.Y << " yaw:" << kesi_new.theta
                 ...
                 << " wf:" << kesi_new.omega_f << " wr:" << kesi_new.omega_r << "\n"
                 << endl;
