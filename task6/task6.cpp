...
{
public:
  float X = 0.0f, Y = 0.0f, theta = 0.0f, v_x = 0.0f, v_y = 0.0f, r = 0.0f, omega_f_l = 0.0f,
        omega_f_r = 0.0f, omega_r_l = 0.0f, omega_r_r = 0.0f, throttle = 0.0f,
        steering_angle = 0.0f, brakes = 0.0f;
};

class States_dot
{
public:
  float X_dot = 0.0f, Y_dot = 0.0f, phi_dot = 0.0f, vx_dot = 0.0f, vy_dot = 0.0f, r_dot = 0.0f,
        omega_dot_f_l = 0.0f, omega_dot_f_r = 0.0f, omega_dot_r_l = 0.0f, omega_dot_r_r = 0.0f;

  States_dot operator*=(float scalar)
  {
    return States_dot{this->X_dot * scalar,         this->Y_dot * scalar,
                      this->phi_dot * scalar,       this->vx_dot * scalar,
                      this->vy_dot * scalar,        this->r_dot * scalar,
                      this->omega_dot_f_l * scalar, this->omega_dot_f_r * scalar,
                      this->omega_dot_r_l * scalar, this->omega_dot_r_r * scalar};
  }
...
public:
  MagicTireModel()
  {
    ...
    Ky =
      pKy1 * Fz0 * sin(2 * atan(Fz / (pKy2 * Fz0 * lFz0))) * (1 - pKy3 * abs(gammay)) * lFz0 * lKy;
  ...
  float r_stat() { return R_tire - Fz / VERTICAL_STIFFNESS; }

  float r_eff() { return sin(acos(r_stat() / R_tire)) / acos(r_stat() / R_tire) * R_tire; }
  ...
  float solveFyCombined(float alpha, float kappa)
  {
    ...
    return Dyk * cos(Cyk * atan(Byk * kappa_s - Eyk * (Byk * kappa_s - atan(Byk * kappa_s)))) +
           SVyk;
  }
...
private:
  ...
  float Fdrv, Frrr, Frrf, Fdrag, Fbf, Fbr, Fxfl, Fxfr, Fxrl, Fxrr, Fyfl, Fyfr, Fyrl, Fyrr, alpha_fl,
    alpha_fr, alpha_rl, alpha_rr, kappa_fl, kappa_fr, kappa_rl, kappa_rr;
...
public:
  ...
  States_dot calculateStatesDot(float thr, float ste, float bra, Kesi kesi)
  {
    ...
    // cout<<Fdrv<<" "<<Frrr<<" "<<Frrf<<" "<< Fdrag<<" "<< Fbf<<" "<< Fbr<<" "<< alpha_f<<" "<<
    // alpha_r<<" "<< Fyf<<" "<<Fyr<<" "<<endl;
    ...
    states_dot_dyn.vx_dot =
      1 / m *
      (m * kesi.v_y * kesi.r + (Fxfl + Fxfr) * cos(kesi_new.steering_angle) + Fxrl + Fxrr -
       Tire.sgn(Fxrl) * Frrr - Tire.sgn(Fxfl) * Frrf * cos(kesi_new.steering_angle) - Fdrag -
       (Fyfl + Fyfr) * sin(kesi_new.steering_angle));
    states_dot_dyn.vy_dot = 1 / m *
                            (-m * kesi.v_x * kesi.r + (Fxfl + Fxfr) * sin(kesi_new.steering_angle) +
                             (Fyfl + Fyfr) * cos(kesi_new.steering_angle) + (Fyrl + Fyrr) -
                             Tire.sgn(Fxfl) * Frrf * sin(kesi_new.steering_angle));

    states_dot_dyn.r_dot = 1 / Iz *
                           (((Fyfl + Fyfr) * cos(kesi_new.steering_angle) +
  ...
  void updatestate(float dt)
  {
    ...
    for (int i = 0; i < steps; ++i) {
      ...
      for (int j = 0; j < 50; ++j) {
        ...
        // outputFile<<" Longitudinal FDRV="<<Fdrv<<" FXR="<<Fxr<<" FXF="<<Fxf<<" FRRF="<<Frrf<<"
        // FBF="<<Fbf<<" FBR="<<Fbr<<"\n"; outputFile<<" vlf "<<vlf<<"
        // "<<kesi_old.omega_f*Tire.r_eff()<<" "<<Tire.kappa(vlf,kesi_old.omega_f)<<"\n";
        // outputFile<<" Kappa "<<kappa_f<<" "<<kappa_r<<"\n";
        // outputFile<<" OMEGA_DOT "<<states_dot_dyn.omega_dot_f<<"
        // "<<states_dot_dyn.omega_dot_r<<"\n";
        kesi_old = kesi_new;
      }
      outputFile << "throttle:" << thr << " steeringAngle:" << ste << " brakes:" << bra << endl;
      outputFile << "X:" << kesi_new.X << " Y:" << kesi_new.Y << " yaw:" << kesi_new.theta
                 << " vx:" << kesi_new.v_x << " vy:" << kesi_new.v_y << " r:" << kesi_new.r
                 << " wfl:" << kesi_new.omega_f_l << " wfr:" << kesi_new.omega_f_r
                 << " wrl:" << kesi_new.omega_r_l << " wrr:" << kesi_new.omega_r_r << "\n"
                 << endl;
      cout << "throttle:" << thr << " steeringAngle:" << ste << " brakes:" << bra << endl;
      cout << "X:" << kesi_new.X << " Y:" << kesi_new.Y << " yaw:" << kesi_new.theta
           << " vx:" << kesi_new.v_x << " vy:" << kesi_new.v_y << " r:" << kesi_new.r
           << " wfl:" << kesi_new.omega_f_l << " wfr:" << kesi_new.omega_f_r
           << " wrl:" << kesi_new.omega_r_l << " wrr:" << kesi_new.omega_r_r << "\n"
           << endl;
